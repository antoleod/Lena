(function () {
  ...
  const SETTINGS = window.ASSOCIE_LES_IDEES_SETTINGS || { storageKey: 'gam_associe_les_idees' };
  const LEVELS = window.ASSOCIE_LES_IDEES_LEVELS || [];
  ...
  function handleAnswer(selectedWord, button) {
    ...
    if (isCorrect) {
      ...
      setFeedback('Bravo, c’est la bonne association !', 'success');
      GAM.speak('Bravo !');
    } else {
  ...
  function renderExercise() {
    ...
    elements.prompt.textContent = exercise.prompt;
  ...
  function finishLevel() {
    ...
    elements.container.innerHTML = '🎉';
    elements.prompt.textContent = 'Mission accomplie !';
  }
