ï»¿ï»¿console.log("login.js loaded");
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Intentar recuperar la selecciÃ³n anterior si existe
  ...
  // Configurar ediciÃ³n directa del nombre
  if (previewPlayerName) {
    previewPlayerName.contentEditable = true;
    previewPlayerName.spellcheck = false;
    previewPlayerName.setAttribute('data-placeholder', t('selectedNamePlaceholder', 'Ã‰cris ton prÃ©nom'));
    
    // Evitar que i18n sobrescriba el contenido si el usuario ya escribiÃ³
  ...
  function renderOwnedCosmetics(profile) {
    ...
    items.forEach(item => {
      if (item.type === 'avatar') {
        ...
        const label = document.createElement('div'); label.className = 'owned-item__name'; label.textContent = item.name || item.id;
        const btn = document.createElement('button'); btn.className = 'owned-item__btn'; btn.type='button'; btn.textContent = t('ownedBackgroundSet', 'DÃ©finir');
  ...
  // Configurar avatars para selecciÃ³n
  ...
  // Si la selecciÃ³n anterior existe, aplicarla
  ...
  colors.forEach(color => {
    ...
    color.addEventListener('click', () => {
      ...
      // Efecto de partÃ­culas al elegir color
  ...
  loginBtn.addEventListener('click', () => {
    ...
    if (!isValidName || !selectedAvatar) {
      if (errorEl) {
        errorEl.textContent = t('loginError', "Ã‰cris ton prÃ©nom et choisis un avatar âœ¨");
  ...
  function animateAvatar(avatar) {
    ...
    void avatar.offsetWidth;  // reiniciar animaciÃ³n
  ...
  function selectAvatar(avatarEl, { animate = false } = {}) {
    ...
    // Aplicar color del avatar si el usuario no eligiÃ³ otro
    ...
    if (animate) {
      ...
      // partÃ­culas al seleccionar avatar
  ...
  // Efecto de partÃ­culas dentro de un elemento (avatar o color)
  ...
  // Efecto de partÃ­culas dentro del botÃ³n (login)
  ...
  function syncCalmToggle(enabled) {
    ...
    if (calmStatus) {
      calmStatus.textContent = enabled
        ? t('calmModeOn', 'Calme activÃ©')
        : t('calmModeOff', 'Calme dÃ©sactivÃ©');
    }
...
// ========== FunciÃ³n global de creaciÃ³n de partÃ­culas ==========
// (Si deseas tambiÃ©n que funcione fuera del DOMContentLoaded)
