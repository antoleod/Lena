...
export default function GridActivity({ activity, progress, onComplete, onQuestionChange }) {
  const { t } = useLocale();
  ...
  function handlePick(choiceValue) {
    ...
    recordPlayedExercise({
      exerciseId: activity.id, sourceModule: 'renforcement-grid',
      subject: activity.subject ?? 'renforcement', questionType: 'grid',
  ...
  return (
    <section className="engine-card engine-card--compact engine-card--arcade">
      ...
      {/* Choices — 2-column grid for grid engine */}
